...
async function setUpPage() {
    const usernames = await fetch("files/usernames.json").then((response) => response.json());
    const tags = await fetch("files/tags.json").then((response) => response.json());
    ...
    async function showRecs() {
        ...
        if (bans.length > 0) {
            ...
            if (oldLen != bans.length && typeof (Storage) !== "undefined") {
